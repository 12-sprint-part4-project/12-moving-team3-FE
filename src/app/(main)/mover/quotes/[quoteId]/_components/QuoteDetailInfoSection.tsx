...
export interface QuoteDetailInfoSectionProps {
...
/** 견적 정보 라벨·값 행 목록 생성 */
const buildInfoRows = (detail: QuoteDetailViewModel): InfoRow[] => [
...
/** 견적 상세 정보 섹션 */
export const QuoteDetailInfoSection = ({
  detail,
}: QuoteDetailInfoSectionProps) => {
    ...
    <section className="flex w-full flex-col gap-5 lg:gap-10">
      ...
      <dl className="flex w-full flex-col gap-4 rounded-2xl border border-line-100 bg-background-200 px-5 py-4 md:px-8 md:py-6 lg:px-10 lg:py-8">
        {rows.map((row) => (
          <div
            key={row.label}
            className="flex items-start gap-[2.5rem] text-md-regular lg:gap-8 lg:text-2lg-regular"
          >
            <dt className="w-16 shrink-0 text-gray-300 lg:w-[5.625rem]">
              {row.label}
            </dt>
