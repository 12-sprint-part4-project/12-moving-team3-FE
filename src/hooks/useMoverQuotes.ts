...
/** 목록 응답 → 카드 UI 모델 변환 */
const selectMoverQuotesView = (
  data: QuoteListResponse,
  status: QuoteListStatus
): MoverQuotesViewData => {
  const quotes =
    status === 'SENT'
      ? (data.data.items as SentQuoteListItem[]).map(toSentQuoteCardModel)
      : (data.data.items as RejectedQuoteListItem[]).map(
          toRejectedQuoteCardModel
        );
...
export const useMoverQuotes = ({
  ...
  const query = useQuery({
    ...
    placeholderData: (
      previousData: QuoteListResponse | undefined,
      previousQuery
    ) => {
    ...
    select: (data) => selectMoverQuotesView(data, status),
  });
